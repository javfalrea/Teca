...
function cargarDetalleParticipante() {
	...
	fetch(`http://localhost:9999/participante/buscarPorId?id=${encodeURI(id)}`)
		...
		.then(p => {
			...
			document.getElementById("bandera").innerHTML = "<img title='" + p.pais.nombre + "' src='" + encodeURI(p.pais.bandera) + "' style='width:16px; height:auto; display:block; margin:auto;'>";
			document.getElementById("pais").textContent = p.pais.nombre;
			document.getElementById("imagen").innerHTML = "<img title='" + p.titulo + "' src='" + encodeURI(p.imagen) + "'>";
		})
	...
	fetch(`http://localhost:9999/pelicula/buscarPorDirector?idParticipante=${encodeURI(id)}`)
		...
		.then(p => {
			if (p.length > 0) {
				document.getElementById("encabezadoD").style.display = "block";
			...
			for (let i = 0; i < p.length; i++) {
				...
				li.innerHTML = `
					${p[i].titulo} 
					<button onclick="verDetallesPelicula('${encodeURI(p[i].id)}')" class="botonPart">Detalles</button>
				`;
	...
	fetch(`http://localhost:9999/pelicula/buscarPorActor?idParticipante=${encodeURI(id)}`)
		...
		.then(p => {
			if (p.length > 0) {
				document.getElementById("encabezadoA").style.display = "block";
	...
	fetch(`http://localhost:9999/serie/buscarPorDirector?idParticipante=${encodeURI(id)}`)
		...
		.then(s => {
			...
			for (let i = 0; i < s.length; i++) {
				...
				li.innerHTML = `
					${s[i].titulo} 
					<button onclick="verDetallesSerie('${encodeURI(s[i].id)}')" class="botonPart">Detalles</button>
				`;
	...
	fetch(`http://localhost:9999/libro/buscarPorAutor?idParticipante=${encodeURI(id)}`)
		...
		.then(l => {
			...
			for (let i = 0; i < l.length; i++) {
				...
				li.innerHTML = `
					${l[i].titulo} 
					<button onclick="verDetallesLibro('${encodeURI(l[i].id)}')" class="botonPart">Detalles</button>
				`;
...
function modificarParticipante() {
	...
	let url = `http://localhost:9999/participante/modificar?id=${encodeURI(id)}&nombre=${encodeURI(nombre)}&fechaNacimiento=${encodeURI(fechaNacimiento)}${fechaFallecimiento ? `&fechaFallecimiento=${encodeURI(fechaFallecimiento)}` : ""}&imagen=${encodeURI(imagen)}&idPais=${encodeURI(pais)}`;
