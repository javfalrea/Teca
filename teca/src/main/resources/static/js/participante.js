	...
	fetch("http://localhost:9999/participante/buscarPorNombre?nombre=" + nombre)
		...
		.then(json => {
			...
			posts.forEach(fila => {
				...
				tabla += "<td><button onclick=\"eliminarParticipante('" + fila.id + "')\">Eliminar</button><button onclick=\"verDetallesParticipante('" + fila.id + "')\">Detalles</button></td>";
				tabla += "</tr>";
			});
...
function gestionarParticipante() {
	...
	var fechaFallecimientoRaw = document.getElementById("fechaFallecimiento").value;
	...
	var url = "http://localhost:9999/participante/crear?nombre=" + nombre + 
			  "&fechaNacimiento=" + fechaNacimiento + 
			  (fechaFallecimientoRaw ? "&fechaFallecimiento=" + encodeURIComponent(fechaFallecimientoRaw) : "") + 
			  "&imagen=" + imagen + 
			  "&idPais=" + pais;
