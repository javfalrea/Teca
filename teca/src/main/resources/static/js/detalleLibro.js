...
function generarEstrellas(valoracion) {
    ...
    const urlEstrellaLlena = "https://png.pngtree.com/png-clipart/20230330/original/pngtree-shiny-yellow-star-icon-clipart-png-image_9011244.png";
	...
	fetch(`http://localhost:9999/participante/buscarPorLibro?id=${encodeURI(id)}`)
		...
		.then(a => {
			for (let i = 0; i < a.length; i++) {
				...
				`;
				document.getElementById("autor").appendChild(li);
			}
		})
		.catch(e => console.log("Error: " + e.message));
...
function gestionarValoracion() {
	...
	if (valorada) {
		url = "http://localhost:9999/libroValoracion/modificar?idLibro=" + encodeURI(id) + "&valoracion=" + encodeURI(valoracion) + "&fav=" + encodeURI(fav) + "&critica=" + encodeURI(critica);
		metodo = { method: 'PUT' };
	} else {
		url = "http://localhost:9999/libroValoracion/crear?idLibro=" + encodeURI(id) + "&valoracion=" + encodeURI(valoracion) + "&fav=" + encodeURI(fav) + "&critica=" + encodeURI(critica);
		metodo = { method: 'POST' };
	}
...
function abrirModificarLibro() {
	...
	fetch(`http://localhost:9999/libro/buscarPorId?id=${encodeURI(id)}`)
		...
		.then(p => {
			...
			document.getElementById("anioMP").value = p.anioLanzamiento;
			document.getElementById("paginasAproxMP").value = p.paginasAprox;
...
function modificarLibro() {
	var idLibro = getIdLibroDesdeUrl();
	...
	var url = "http://localhost:9999/libro/modificar?id=" + idLibro + "&titulo=" + titulo + "&tituloOriginal=" + tituloOriginal + "&anioLanzamiento=" + anioLanzamiento + "&paginasAprox=" + paginasAprox + "&sinopsis=" + sinopsis + "&imagen=" + imagen;
