	...
	fetch("http://localhost:9999/serie/busquedaAvanzada?tituloBq=" + titulo + "&participanteBq=" + participante + "&idGenero=" + genero)
		...
		.then(json => {
			...
			const promesas = posts.map(fila => {
				...
				const promesaTemporadas = fetch("http://localhost:9999/temporada/buscarPorSerie?idSerie=" + idSerie) // corregido aquí
					.then(res => res.text())
					.then(jsonTemp => {
						try {
...
function gestionarSerie() {
	...
	var url = "http://localhost:9999/serie/crear?titulo=" + titulo + "&tituloOriginal=" + tituloOriginal +  "&terminada=" + terminada + "&idPais=" + pais + "&sinopsis=" + sinopsis + "&imagen=" + imagen;
