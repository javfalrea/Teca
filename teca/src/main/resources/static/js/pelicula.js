	...
	fetch("http://localhost:9999/pelicula/busquedaAvanzada?tituloBq=" + titulo + "&participanteBq=" + participante + "&idGenero=" + genero)
		...
		.then(json => {
			...
			const promesas = posts.map(fila => {
				return fetch("http://localhost:9999/participante/buscarDirectorPorPelicula?id=" + encodeURIComponent(fila.id))
					.then(res => res.text())
					.then(jsonDir => {
						const directores = JSON.parse(jsonDir);
						let directorTexto = "ND";
						if (directores && directores.length > 0) {
							directorTexto = directores.map(d => d.nombre).join(", ");
						}

						let filaHTML = "<tr>";
						filaHTML += "<td>" + fila.titulo + "</td>";
						filaHTML += "<td>" + fila.anioEstreno + "</td>";
						...
						filaHTML += "<td><button onclick=\"eliminarPelicula('" + fila.id + "')\">Eliminar</button><button onclick=\"verDetallesPelicula('" + fila.id + "')\">Detalles</button></td>";
						filaHTML += "</tr>";
