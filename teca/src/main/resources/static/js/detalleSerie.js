...
function cargarDetalleSerie() {
	...
	fetch(`http://localhost:9999/serie/buscarPorId?id=${encodeURI(id)}`)
		...
		.then(p => {
			...
			if(p.terminada) {
				...
				document.getElementById("botonCrearTemporada").style.display = "block";
			}
	...
	fetch(`http://localhost:9999/temporada/buscarPorSerie?idSerie=${encodeURI(id)}`)
		.then(res => res.json())
		.then(t => {
			for (let i = 0; i < t.length; i++) {
				...
				`;
				document.getElementById("temporada").appendChild(li);
			}
		})
		.catch(e => console.log("Error: " + e.message));
	...
	// Cargar valoración
	fetch(`http://localhost:9999/serieValoracion/buscarPorId?idSerie=${encodeURI(id)}`)
		.then(res => res.json())
		.then(vp => {
			valorada = true;
...
function abrirModificarValoracion() {
	...
	document.getElementById("idSerie").value = getIdSerieDesdeUrl();
...
function verDetallesTemporada(idTemporada) {
	window.location.href = `detalleTemporada.html?id=${encodeURI(idTemporada)}`;
}
...
function eliminarParticipante(idParticipante) {
	...
	fetch("http://localhost:9999/serieParticipante/desagregar?idSerie=" + encodeURI(idSerie) + "&idParticipante=" + encodeURI(idParticipante), { method: 'DELETE' })
		.then(res => res.text())
		.then(res => {
			cargarDetalleSerie();
		})
...
function modificarSerie() {
	...
	var terminada = document.getElementById("terminadaMP").checked;
...
function cerrarCrearTemporada() {
	...
	document.getElementById("anioEstrenoT").value = "";
	document.getElementById("numCapitulosT").value = "";
	document.getElementById("duracionMediaT").value = "";
	document.getElementById("sinopsisT").value = "";
...
function eliminarTemporada(idTemporada) {
	if (!confirm("Confirma si quieres suprimir esta temporada")) {
		return;
	}
	fetch("http://localhost:9999/temporada/eliminar?id=" + encodeURI(idTemporada), { method: 'DELETE' })
		.then(res => res.text())
		.then(res => {
			cargarDetalleSerie();
		})
